...
| `update_flight` | `PATCH /api/v3/flight` — partial `data` merge. **X-Plane does not allow start-location keys on PATCH** (`lle_ground_start`, `lle_air_start`, `runway_start`, `ramp_start`, `boat_start` → HTTP 400 *invalid to specify a start*). Use `start_flight` (POST) to set lat/lon/altitude via `lle_*`. |
